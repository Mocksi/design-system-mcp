...
export class TokenFileDiscovery {
    ...
    discoverTokenFiles() {
        ...
        if (!existsSync(directory)) {
            discoveryErrors.push(`Token directory not found: ${directory}. ` +
        ...
        if (files.length === 0) {
            discoveryErrors.push(`No token files found in ${directory}. ` +
    ...
    scanTokenFiles(directory, errors) {
        const files = [];
        try {
    ...
    shouldScanSubdirectory(dirname) {
    ...
    static getRecommendedTokenDirectory() {
        return resolve('./design-system-mcp/tokens');
    }
    static createTokenDirectoryStructure(baseDir) {
    ...
    static validateTokenDirectory(directory) {
        const issues = [];
        if (!existsSync(directory)) {
            return {
                isValid: false,
...
export function discoverDesignTokens(directory) {
    const discovery = new TokenFileDiscovery(directory);
    return discovery.discoverTokenFiles();
}
export function getTokenFilesInDirectory(directory) {
    const discovery = new TokenFileDiscovery(directory);
    const result = discovery.discoverTokenFiles();
    return result.files;
}
export function findTokenDirectory() {
...
export function getTokenDiscoverySummary(result) {
...
// Environment helpers
export function getEnvironmentTokenPath() {
    return process.env.DESIGN_TOKENS_PATH;
}
export function setEnvironmentTokenPath(path) {
    process.env.DESIGN_TOKENS_PATH = resolve(path);
}
// File system utilities for token management
export function getTokenFileStats(filePath) {
    try {
...
export function isValidTokenDirectory(directory) {
...
(function (FileErrorType) {
    FileErrorType["DIRECTORY_NOT_FOUND"] = "DIRECTORY_NOT_FOUND";
    FileErrorType["DIRECTORY_NOT_ACCESSIBLE"] = "DIRECTORY_NOT_ACCESSIBLE";
    FileErrorType["FILE_NOT_FOUND"] = "FILE_NOT_FOUND";
    FileErrorType["FILE_NOT_ACCESSIBLE"] = "FILE_NOT_ACCESSIBLE";
    FileErrorType["FILE_CORRUPTED"] = "FILE_CORRUPTED";
    FileErrorType["FILE_EMPTY"] = "FILE_EMPTY";
    FileErrorType["FILE_TOO_LARGE"] = "FILE_TOO_LARGE";
    FileErrorType["INVALID_PERMISSIONS"] = "INVALID_PERMISSIONS";
    FileErrorType["NO_TOKEN_FILES"] = "NO_TOKEN_FILES";
    FileErrorType["SYMLINK_BROKEN"] = "SYMLINK_BROKEN";
})(FileErrorType || (FileErrorType = {}));
export class FileErrorHandler {
    static MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB
    static MIN_FILE_SIZE = 2; // 2 bytes minimum for "{}"
    static analyzeDirectoryError(directory) {
        const errors = [];
        if (!existsSync(directory)) {
            errors.push({
    ...
    static analyzeFileError(filePath) {
    ...
    static generateHelpfulErrorMessage(error) {
    ...
    static getCommonSolutions(errorType) {
        const solutions = {
    ...
    static diagnoseTokenSetup(directory) {
...
export function discoverDesignTokensWithErrorHandling(directory) {
    const discovery = new TokenFileDiscovery(directory);
    const result = discovery.discoverTokenFiles();
...
export function formatFileErrorReport(errors) {
    if (errors.length === 0) {
