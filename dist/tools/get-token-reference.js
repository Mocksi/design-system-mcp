...
import { resolveTokenReferences } from '../parsers/w3c-tokens.js';
...
export async function getTokenReference(args) {
    const { category, name, directory, includeExamples = true, includeRelated = true } = TokenReferenceArgsSchema.parse(args);
    try {
        ...
        if (!token) {
            ...
            throw new McpError(ErrorCode.InvalidRequest, `Token "${name}" not found in category "${category}". Available tokens: ${availableTokens.join(', ')}${moreTokens}`);
        }
...
function generateCSSClass(tokenName) {
    return `.${tokenName.replace(/\./g, '-')}`;
}
function generateSassVariable(tokenName) {
    return `$${tokenName.replace(/\./g, '-')}`;
}
function generateLessVariable(tokenName) {
    return `@${tokenName.replace(/\./g, '-')}`;
}
function generateJSConstant(tokenName) {
    return tokenName.replace(/[\.-]/g, '_').toUpperCase();
}
function getDetailedUsageInformation(type, path, value) {
    if (!type)
        return 'General purpose design token';
    const baseUsage = {
...
function getSemanticContext(path, type) {
    const semanticMap = {
        color: {
...
function generateCodeExamples(type, name, value) {
...
function generatePlatformUsage(type, name, value) {
...
function findRelatedTokens(currentToken, allTokens) {
    const related = [];
...
function determineTokenRelationship(token1, token2) {
    // Same type tokens
...
function analyzeTokenHierarchy(currentToken, allTokens) {
    const hierarchy = {
        children: [],
        variants: [],
    };
...
function generateRecommendations(tokenRef) {
...
function generateNextSteps(tokenRef, category) {
    const nextSteps = [];
    // Immediate actions
...
// Validation helpers
export function validateTokenReferenceRequest(args) {
    const issues = [];
