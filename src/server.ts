...
import { getDesignSystemInfo } from './tools/get-design-system-info.js';
import { getCategoryTokens } from './tools/get-category-tokens.js';
import { getTokenReference } from './tools/get-token-reference.js';
...
Progressive flow: Use after get_category_tokens to get detailed information about specific tokens you want to implement.`,
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'get_design_system_info':
            return await wrapToolExecution(
              ...
              this.handleGetDesignSystemInfo.bind(this),
              validateLayer1Args
            )(args);
          ...
          case 'get_category_tokens':
            return await wrapToolExecution(
              'get_category_tokens', 
              this.handleGetCategoryTokens.bind(this),
              validateLayer2Args
            )(args);
          ...
          case 'get_token_reference':
            return await wrapToolExecution(
              'get_token_reference',
              this.handleGetTokenReference.bind(this),
              validateLayer3Args
            )(args);
          ...
          default:
            throw new McpError(
              ...
              `Unknown tool: ${name}. Available tools: get_design_system_info, get_category_tokens, get_token_reference`
            );
        }
      } catch (error) {
        ...
        throw new McpError(
          ...
          `Tool execution failed for "${name}": ${errorMessage}. Please check your parameters and try again.`
        );
      }
    });
  }

  private async handleGetDesignSystemInfo(args: any) {
    ...
    try {
      ...
      if (!healthCheck.isHealthy) {
        const criticalErrors = healthCheck.errors.filter(e => 
          e.type === 'DIRECTORY_NOT_FOUND' || 
        ...
        if (criticalErrors.length > 0) {
          ...
          throw new McpError(
            ...
            `Cannot access design tokens: ${errorMessages.join('; ')}`
          );
        }
      }
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to discover design tokens: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async handleGetCategoryTokens(args: any) {
    // Validation is now handled by wrapToolExecution
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get category tokens: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async handleGetTokenReference(args: any) {
    // Validation is now handled by wrapToolExecution
  ...
  private getTokenUsageHint(type?: string, path?: string[]): string {
    if (!type) return 'General purpose token';
  ...
  private getTokenExamples(type?: string, value?: any, name?: string): string[] {
    if (!type || !name) return [];

    const examples: Record<string, (value: any, name: string) => string[]> = {
      color: (value, name) => [
        `color: var(--${name});`,
  ...
  private findRelatedTokens(currentToken: any, allTokens: any[]): string[] {
    // Find tokens with similar paths or names
    const related = allTokens
      .filter(token => 
        token.name !== currentToken.name && (
          token.path[0] === currentToken.path[0] || // Same top-level category
