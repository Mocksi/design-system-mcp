...
export async function getDesignSystemInfo(args: DesignSystemInfoArgs): Promise<DesignSystemInfoResult> {
  ...
  try {
    ...
    if (!healthCheck.isHealthy) {
      ...
      if (criticalErrors.length > 0) {
        const errorMessages = criticalErrors.map(e => {
          let msg = e.message;
          if (e.suggestion) msg += ` Suggestion: ${e.suggestion}`;
          if (e.recoveryAction) msg += ` Recovery: ${e.recoveryAction}`;
          return msg;
        });
    ...
    const categoryDetails: CategorySummary[] = result.parsedResult.categories.map(cat => {
    ...
    throw new McpError(
      ...
      `Failed to discover design tokens in ${errorContext}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}

// Helper function to get setup guidance based on discovery results
export function getSetupGuidance(result: DesignSystemInfoResult): string[] {
...
export function validateDiscoveryRequest(args: any): string[] {
  const issues: string[] = [];
...
export function formatDiscoveryResponse(result: DesignSystemInfoResult, format: 'json' | 'summary' | 'detailed' = 'json'): string {
  switch (format) {
    case 'summary':
